...
class AgencyService {
  ...
  Future<Map<String, dynamic>> getMonthlyReport(String agencyId) async {
    ...
    final supportQuery = await _db
        .collection('guild_support_logs')
    ...
    for (var doc in supportQuery.docs) {
      ...
      if (data['currency'] == 'gems') {
        totalGems += (data['amount'] as num? ?? 0).toInt();
      } else if (data['currency'] == 'stars' || data['currency'] == 'coins') {
        totalStars += (data['amount'] as num? ?? 0).toInt();
      }
    }
  ...
  Future<void> createAgencyByShortId({
    ...
    await _db.runTransaction((tx) async {
      ...
      tx.update(userDoc.reference, {
        ...
        'agencyType': type == AgencyType.reseller
            ? 'official_support'
            : 'community_hosting',
      });
    });
  ...
  Future<void> transferCoinsToUser(String targetShortId, int amount) async {
    ...
    final String agentAvatar =
        agentDoc.data()?['photoUrl'] ?? agentDoc.data()?['photoURL'] ?? '';
    ...
    await _db.runTransaction((tx) async {
      ...
      tx.set(notifyRef, {
        'title': "دعم ملكي جديد 🪙",
        'body':
            "قام قائد بيت الدعم $agentName بمنحك $amount كوينز 🪙 كدعم لنمو حسابك.",
        'type': 'guild_support',
        'senderId': agentUid,
  ...
  Future<void> addMemberToAgency(String userShortId) async {
    ...
    final userQuery = await _db
        .collection('users')
        .where('shortId', isEqualTo: userShortId.toUpperCase())
        .limit(1)
        .get();
    if (userQuery.docs.isEmpty) throw 'المستخدم غير موجود';
    final userDoc = userQuery.docs.first;

    if (userDoc.data()['agencyId'] != null) {
      throw 'هذا المستخدم منضم لوكالة أخرى بالفعل';
    }

    await _db.runTransaction((tx) async {
      tx.update(userDoc.reference, {'agencyId': agencyId});
      tx.update(_db.collection('agencies').doc(agencyId),
  ...
  Stream<AgencyModel?> watchMyAgency() {
    ...
    return _db
      ...
      return AgencyModel.fromFirestore(
