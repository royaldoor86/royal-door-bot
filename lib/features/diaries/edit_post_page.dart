...
class _EditPostPageState extends State<EditPostPage> {
  ...
  final List<dynamic> _mediaList = []; // Can contain String (URLs) or File (Local)
  ...
  Future<void> _pickImages() async {
    ...
    if (images.isNotEmpty) {
      setState(() {
        // If we add images, we clear video to maintain post type consistency (optional, but standard in some apps)
        // In your create_post_page.dart, you clear video when picking images.
        _newVideoFile = null;
  ...
  Future<void> _pickVideo() async {
    ...
    if (video != null) {
      setState(() {
        _mediaList.clear(); // Creating post logic cleared images when video picked
  ...
  Future<void> _handleUpdate() async {
    if (_textController.text.trim().isEmpty && _mediaList.isEmpty && _newVideoFile == null && _existingVideoUrl == null) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('المنشور لا يمكن أن يكون فارغاً')));
        return;
    }
    ...
    try {
      ...
      if (_newVideoFile != null) {
        finalVideoUrl = await StorageService.uploadDailyPostVideo(_newVideoFile!);
      }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم تحديث المنشور بنجاح ✅')));
      }
    } catch (e) {
      debugPrint("Update Post Error: $e");
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('حدث خطأ أثناء التحديث')));
    } finally {
        ...
        appBar: AppBar(
          ...
          actions: [
            Padding(
              ...
              child: _isLoading
                  ? const Center(child: SizedBox(width: 24, height: 24, child: CircularProgressIndicator(color: AppTheme.royalGold, strokeWidth: 2)))
                  : ElevatedButton(
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    if (_newVideoFile != null || _existingVideoUrl != null) _buildVideoPreview(),
  ...
  Widget _buildVideoPreview() {
    return Container(
      ...
      child: Stack(alignment: Alignment.center, children: [
        ...
        Positioned(bottom: 15, child: Text(_newVideoFile != null ? "فيديو جديد جاهز" : "الفيديو الحالي", style: const TextStyle(color: Colors.white70, fontSize: 12))),
      ]),
    );
  }
        ...
        return Stack(fit: StackFit.expand, children: [
          ClipRRect(
              ...
              child: item is File 
                ...
                : CachedNetworkImage(imageUrl: item, fit: BoxFit.cover, placeholder: (c,u) => Container(color: Colors.white10))),
          Positioned(
              top: 5,
              right: 5,
