...
import '../user_gifts_page.dart';
import '../user_vehicles_page.dart';
...
class _UserDetailsViewPageState extends State<UserDetailsViewPage>
  ...
  Future<void> _showChangeHonoraryTitleDialog() async {
    if (_currentUserId != widget.user.uid) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('لا يمكنك تغيير لقب مستخدم آخر')),
      );
      return;
    }

    final TextEditingController titleController = TextEditingController(
      text: widget.user.honoraryTitle ?? '',
    );
        ...
        content: TextField(
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(12),
              borderSide: const BorderSide(color: AppTheme.royalGold, width: 2),
            ),
          ),
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (newTitle.isEmpty) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('اللقب لا يمكن أن يكون فارغاً')),
                );
                return;
              }
              ...
              try {
                await FirebaseFirestore.instance
                    .collection('users')
                    .doc(_currentUserId)
                    .update({'honoraryTitle': newTitle});
                if (mounted) {
                  // تحديث البيانات محلياً
                  setState(() {
                    widget.user.honoraryTitle = newTitle;
                  });
                  Navigator.pop(context);
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                        content: Text('تم تحديث اللقب الملكي بنجاح 👑')),
                  );
                }
              } catch (e) {
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('فشل تحديث اللقب: $e')),
                  );
                }
              }
            },
      ...
      children: [
        Padding(
            ...
            child: _buildSectionHeader('اللقب الملكي الشرفي', 'تغيير',
                onTap: _showChangeHonoraryTitleDialog)),
        Center(
            child: RankBadge(
        ...
        Padding(
            padding: const EdgeInsets.all(20),
            child: _buildSectionHeader('سجل المعارك', '')),
        _buildBattleStats(),
        Padding(
            padding: const EdgeInsets.all(20),
            child: _buildSectionHeader(
                'المركبات الخاصة (${_userVehicles.length})', 'عرض الكل',
        ...
        Padding(
            ...
            child: _buildSectionHeader(
                ...
                onTap: () => Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (_) =>
                            UserGiftsPage(userId: widget.user.uid))))),
        if (_userGifts.isEmpty)
          const Center(
              child: Text('لا توجد هدايا مستلمة حتى الآن',
                  style: TextStyle(color: Colors.white38, fontSize: 13)))
        else
          GridView.builder(
              shrinkWrap: true,
              ...
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                  crossAxisCount: 4, mainAxisSpacing: 12, crossAxisSpacing: 12),
              itemCount: _userGifts.length > 8 ? 8 : _userGifts.length,
              itemBuilder: (context, index) {
                final gift = _userGifts[index];
                return GiftItem(
                    index: index,
                    icon: Icons.card_giftcard,
                    color: Colors.pinkAccent,
                    name: gift['name'] ?? 'هدية ملكية',
                    imageUrl: gift['imageUrl'],
                    count: gift['count'] ?? 1,
                    onTap: () => _showGiftDetails(
                        gift['name'] ?? 'هدية ملكية', Colors.pinkAccent));
              }),
      ],
    );
  }
  ...
  Widget _buildSectionHeader(String title, String trailing,
      {VoidCallback? onTap}) {
    return SectionHeader(title: title, trailing: trailing, onTap: onTap);
  }
  ...
  Widget _buildBattleStats() {
    return BattleStatsWidget(
      battleWins: widget.user.battleWins,
      battleLosses: widget.user.battleLosses,
    );
  }
