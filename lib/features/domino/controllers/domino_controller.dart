...
class DominoController extends ChangeNotifier {
  ...
  bool get canCurrentPlayerMove {
    ...
    return hand.any((t) =>
        t.left == board.first.left ||
        t.right == board.first.left ||
        t.left == board.last.right ||
        t.right == board.last.right);
  }
  ...
  void initGame(int playerCount,
      {GameMode mode = GameMode.solo, PlayType type = PlayType.offline}) {
    gameMode = mode;
    playType = type;
    board.clear();
    stock.clear();
    ...
    for (int i = 0; i <= maxTileValue; i++) {
      for (int j = i; j <= maxTileValue; j++) {
        allTiles.add(
            DominoTile(id: 'tile-$i-$j', left: i, right: j, isDouble: i == j));
      }
    }
    allTiles.shuffle();
    ...
    players = List.generate(
        ...
        (index) => DominoPlayer(
              id: index == 0
                  ? (_auth.currentUser?.uid ?? 'player-0')
                  : 'ai-$index',
    ...
    players[startingPlayerIdx]
        .hand
        .removeWhere((t) => t.id == startingTile!.id);
    ...
    if (playType == PlayType.offline &&
        players[currentPlayerIndex].id.startsWith('ai')) {
  ...
  void _syncFromFirebase(Map<String, dynamic> data) {
    if (data['board'] != null) {
      board = (data['board'] as List)
          .map((e) => DominoTile.fromJson(Map<String, dynamic>.from(e)))
          .toList();
      _updateBoardPositions();
    }
  ...
  void _updateBoardPositions() {
    ...
    // Simple linear layout - all tiles in a straight horizontal line
    ...
    for (int i = 0; i < board.length; i++) {
      ...
      // All tiles horizontal for now
      tile.rotation = 0;
      tile.x = curX;
      tile.y = 0;

      // Move to next position
      double tileLength = tile.isDouble ? tileW : tileH;
      curX += tileLength + spacing;
    }

    if (board.isNotEmpty) {
      DominoTile first = board.first;
      DominoTile last = board.last;
      nextLeftPos = Offset(first.x - 50, first.y);
      nextRightPos = Offset(last.x + 50, last.y);
    }
  }
  ...
  void _checkGameBlock() {
    ...
    for (var player in players) {
      for (var tile in player.hand) {
        if (tile.left == board.first.left ||
            tile.right == board.first.left ||
            tile.left == board.last.right ||
            tile.right == board.last.right) {
          canAnyoneMove = true;
          break;
        }
      }
      ...
      try {
        ...
        await walletRef.runTransaction((Object? wallet) {
          ...
          Map<String, dynamic> walletData =
              Map<String, dynamic>.from(wallet as Map);
          walletData['coins'] =
              (walletData['coins'] ?? 0) + (config['coins'] * 2);
