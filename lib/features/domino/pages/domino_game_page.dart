...
class _RoyalDominoPageState extends State<RoyalDominoPage>
  ...
  void _fitCameraToBoard(List<DominoTile> tiles) {
    ...
    double scaleY =
        (screenSize.height - 350) / boardHeight; // Account for top/bottom UI
    ...
    final Matrix4 endMatrix = Matrix4.identity()
      ...
      ..translate(-1000.0 - centerX + (screenSize.width / (2 * targetScale)),
  ...
  Widget _buildPlayerInfo(DominoController game, {required bool isTop}) {
    final player = isTop
        ? (game.players.length > 1 ? game.players[1] : null)
        : (game.players.isNotEmpty ? game.players[0] : null);
      ...
      child: Column(
        children: [
              ...
              if (player.isCurrentPlayer)
                const SizedBox(
                  ...
                  child: CircularProgressIndicator(
                    ...
                    strokeWidth: 3,
                    valueColor:
                        AlwaysStoppedAnimation<Color>(DesignTokens.primaryGold),
                  ),
                ),
              Container(
                width: 60,
          ...
          Text("Points: ${player.score}",
              style: const TextStyle(
                  color: DesignTokens.primaryGold, fontSize: 11)),
        ],
      ),
    );
  }
  ...
  Widget _buildPositionedDropZone(DominoController game, String side) {
    ...
    if (side == 'center') {
      ...
      x = game.nextLeftPos.dx;
      y = game.nextLeftPos.dy;
    } else if (side == 'right') {
      x = game.nextRightPos.dx;
      y = game.nextRightPos.dy;
    }
    ...
    return Positioned(
      ...
      child: GestureDetector(
        onTap: () =>
            game.playMove(game.selectedTile!, side == 'center' ? 'left' : side),
        child: Container(
          width: 40,
          height: 40,
          decoration: BoxDecoration(
  ...
  void _showWinOverlay(BuildContext context, DominoController game) {
    showGeneralDialog(
      ...
      pageBuilder: (context, _, __) {
        ...
        final bool isWinner = game.winner?.id ==
            (game.players.isNotEmpty ? game.players[0].id : '');
        return Center(
          child: Material(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(isWinner ? "لقد انتصرت!" : "حظاً أوفر",
                    style: const TextStyle(
                        color: Colors.white,
                        fontSize: 32,
                        fontWeight: FontWeight.bold)),
                const SizedBox(height: 20),
                Text("Gems: ${rewards['gems']} | Coins: ${rewards['coins']}",
