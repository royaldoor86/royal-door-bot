      ...
      builder: (ctx) => SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              const HeadingText('معلومات الرسالة',
                ...
                StreamBuilder<DocumentSnapshot>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      if (vipRank.isNotEmpty) {
                        return const SizedBox.shrink(); // لا إعلانات للـ VIP
                      }
  ...
  Widget _buildPinnedMessageBar() {
    ...
    return Container(
      ...
      padding: const EdgeInsets.symmetric(
          horizontal: DesignTokens.spacingLg, vertical: DesignTokens.spacingSm),
      decoration: const BoxDecoration(
        color: DesignTokens.backgroundDarkMedium,
        border: Border(
            bottom: BorderSide(color: DesignTokens.primaryGold, width: 1)),
      ),
      child: InkWell(
              ...
              children: [
                CircleAvatar(
                    ...
                    backgroundImage: (widget.otherUser.profilePic.isNotEmpty &&
                            Uri.tryParse(widget.otherUser.profilePic)
                                    ?.host
                                    .isNotEmpty ==
                                true)
                        ? CachedNetworkImageProvider(
            ...
            children: [
              ...
              ListTile(
                leading: const Icon(Icons.reply_rounded,
                    color: DesignTokens.primaryGold),
                title:
                    const BodyText('رد سريع', color: DesignTokens.neutralWhite),
                onTap: () {
                  Navigator.pop(ctx);
            ...
            children: [
                ...
                onTap: () async {
                  ...
                  bool? confirm = await showDialog<bool>(
                    ...
                    builder: (ctx) => RoyalConfirmDialog(
                      title: 'مسح الدردشة',
                      message:
                          'هل أنت متأكد من مسح كافة الرسائل؟ ستبقى المحادثة موجودة.',
                      onConfirm: () => Navigator.pop(ctx, true),
                ...
                onTap: () async {
                  ...
                  bool? confirm = await showDialog<bool>(
                    ...
                    builder: (ctx) => RoyalConfirmDialog(
                      title: 'حذف المحادثة',
                      message:
                          'سيتم حذف المحادثة من قائمتك نهائياً. يمكنك مراسلته مرة أخرى لاحقاً.',
                      onConfirm: () => Navigator.pop(ctx, true),
              ...
              ListTile(
                leading: const Icon(Icons.block_rounded,
                    color: DesignTokens.semanticError),
                title: const BodyText('حظر المستخدم',
                    color: DesignTokens.semanticError),
                onTap: () async {
                  Navigator.pop(context);
                  bool? confirm = await showDialog<bool>(
                    ...
                    builder: (ctx) => RoyalConfirmDialog(
                      title: 'حظر المستخدم',
                      message:
                          'هل أنت متأكد من حظر ${widget.otherUser.name}؟ لن يتمكن من مراسلتك مرة أخرى.',
                      confirmLabel: 'حظر',
                      onConfirm: () => Navigator.pop(ctx, true),
                  ...
                  if (confirm == true) {
                    await _firestoreService.blockUser(
                        _currentUserId, widget.otherUser.uid);
                    if (!mounted) return;
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              ...
              ListTile(
                  ...
                  onTap: () async {
                    ...
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                          content: Text(
                              'تم إرسال بلاغ عن هذا المستخدم للإدارة 🛡️')));
                    }
                  }),
              const SizedBox(height: DesignTokens.spacingMd),
            ],
  ...
  Widget _buildReplyPreview() {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          border: const Border(
              top: BorderSide(color: DesignTokens.primaryGold, width: 0.5))),
      child: Row(
        children: [
          ...
          IconButton(
              icon: const Icon(Icons.close,
                  size: 18, color: DesignTokens.neutralGray600),
                    ...
                    child: Container(
                      ...
                      child: TextField(
                        ...
                        decoration: const InputDecoration(
                            ...
                            hintStyle:
                                TextStyle(color: DesignTokens.neutralGray600),
                            border: InputBorder.none),
                      ),
                    ),
                  ),
      ...
      builder: (context) => SafeArea(
        child: SingleChildScrollView(
          ...
          child: Row(
            ...
            children: [
              _mediaOption(Icons.image_rounded, 'صور',
                  DesignTokens.semanticSuccess, _uploadAndSendImage),
              _mediaOption(Icons.videocam_rounded, 'فيديو',
                  DesignTokens.primaryGold, _pickAndSendVideo),
            ],
          ),
        ),
      ),
    ...
    return InkWell(
      ...
      child: Column(
        ...
        children: [
          ...
          CaptionText(label, color: DesignTokens.neutralGray400, fontSize: 12),
        ],
      ),
    );
  }
