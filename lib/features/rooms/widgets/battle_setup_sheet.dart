...
class _BattleSetupSheetState extends State<BattleSetupSheet> {
  ...
  int _selectedRounds = 1; // عدد الجولات
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border:
                Border.all(color: isSelected ? Colors.amber : Colors.white10),
          ),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Icon(icon,
                  color: isSelected ? Colors.amber : Colors.white38, size: 18),
  ...
  Widget _buildRivalSelection() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (users.isEmpty) {
          return const Text('لا يوجد مستخدمون آخرون في الغرفة حالياً',
  ...
  Widget _buildRoundsSelection() {
    return Row(
      children: [1, 3, 5].map((r) {
        bool isSelected = _selectedRounds == r;
        return Expanded(
          child: GestureDetector(
  ...
  void _startBattle() async {
    if (_battleMode == 'individual' && _selectedRivalId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('يرجى اختيار الخصم أولاً!')));
      return;
    }
    ...
    await FirebaseFirestore.instance
      ...
      'battle': {
        ...
        'blueName':
            _battleMode == 'individual' ? _selectedRivalName : 'الفريق الأزرق',
  ...
  Widget _buildStartButton() {
    return Container(
      ...
      child: ElevatedButton(
        ...
        child: const Text('بدء التحدي الملكي ⚔️',
            style: TextStyle(
                color: Colors.white,
                fontSize: 18,
                fontWeight: FontWeight.bold)),
