...
class _GiftShopSheetState extends State<GiftShopSheet> {
  ...
  Widget _buildGiftItemFirestore(int index, DocumentSnapshot doc) {
    ...
    final int? battlePoints = d['battlePoints'];
      ...
      child: AnimatedContainer(
        ...
        child: Column(
          ...
          children: [
            ...
            // عرض نقاط المعركة
            if (battlePoints != null && battlePoints > 0)
              Padding(
  ...
  Future<void> _sendGift(BuildContext context) async {
    ...
    try {
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        ...
        // تحديث عدد الهدايا المستلمة للمستلم
        if (_selectedReceiverId != 'all' && _selectedReceiverId != null) {
          final receiverRef = FirebaseFirestore.instance.collection('users').doc(_selectedReceiverId);
          transaction.update(receiverRef, {
            'totalGiftsReceived': FieldValue.increment(_giftCount),
          });
        }
        ...
        if (roomSnap.exists) {
          ...
          if (battle != null && battle['active'] == true) {
            ...
            if (pointsToAdd > 0) {
                ...
                if (_selectedReceiverId == 'all') {
                  ...
                  if (redOnMic > 0) {
                    transaction.update(roomRef, {
                      'battle.redPoints': FieldValue.increment(pointsToAdd),
                      'battle.redStars':
                          FieldValue.increment(pointsToAdd), // Modern sync
                      'battle.redPool': FieldValue.increment(pointsToAdd)
                    });
                  }
                  if (blueOnMic > 0) {
                    transaction.update(roomRef, {
                      'battle.bluePoints': FieldValue.increment(pointsToAdd),
                      'battle.blueStars':
                          FieldValue.increment(pointsToAdd), // Modern sync
                      'battle.bluePool': FieldValue.increment(pointsToAdd)
                    });
                  }
                } else {
              ...
              if (team != null) {
                transaction.update(roomRef, {
                  'battle.${team}Points': FieldValue.increment(pointsToAdd),
                  'battle.${team}Stars':
                      FieldValue.increment(pointsToAdd), // Modern sync
                  'battle.${team}Pool': FieldValue.increment(pointsToAdd),
                });
              }
            }
          }
