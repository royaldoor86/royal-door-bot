...
class _InvitesPageState extends State<InvitesPage> {
  ...
  void _copyCode(String code) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: const Text('تم نسخ كود الدعوة الملكي بنجاح 📋',
    ...
    return Directionality(
      ...
      child: StreamBuilder<UserModel>(
          ...
          builder: (context, snapshot) {
            ...
            int totalGems =
                (userData.agentData?['referralEarnings'] ?? 0).toInt();
              ...
              body: Container(
                decoration: const BoxDecoration(
                  image: DecorationImage(
                    image: AssetImage(
                        'assets/images/auth_bg.png'), // خلفية ملكية إن وجدت
                    fit: BoxFit.cover,
                    opacity: 0.1,
                  ),
                ),
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            const Text('حدث خطأ في تحميل البيانات الملكية',
  ...
  Widget _buildStatsGrid(int count, int gems) {
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(
              child: _statCard('أصدقاء الدعوة', count.toString(),
                  Icons.group_add, Colors.blueAccent)),
          const SizedBox(width: 15),
          Expanded(
              child: _statCard('أرباح الجواهر', gems.toString(),
                  Icons.diamond_rounded, Colors.cyanAccent)),
          const SizedBox(width: 15),
          Expanded(
              child: _statCard('أرباح الكوينز', (count * 5).toString(),
                  Icons.monetization_on, Colors.orangeAccent)),
        ],
      ),
    );
  }
      ...
      child: Column(
        children: [
          const Align(
            ...
            child: Text('هويتك الملكية للدعوات:',
          ...
          const SizedBox(height: 10),
          const Text(
              'شارك هذا الكود مع أصدقائك للحصول على 5 جواهر و 5 كوينز عن كل عملية تسجيل',
              textAlign: TextAlign.center,
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            color:
                completed ? Colors.amber : Colors.white.withValues(alpha: 0.05),
            boxShadow: completed
                ? [
                    BoxShadow(
        ...
        Text(name,
            style: TextStyle(
                color: completed ? Colors.amber : Colors.white24, fontSize: 9)),
      ],
    );
  }
            ...
            child: Column(
              children: [
                Icon(Icons.person_add_disabled_rounded,
                    color: Colors.white.withValues(alpha: 0.05), size: 60),
                const SizedBox(height: 15),
                const Text(
                    'لم ينضم أحد بعد.. كن أول من يفتح بوابة المملكة لأصدقائه!',
                    textAlign: TextAlign.center,
                  ...
                  child: ListTile(
                    leading: CircleAvatar(
                        ...
                        child: (friend['profilePic'] == null ||
                            ...
                            : null),
                    title: Text(friend['name'] ?? 'مستخدم جديد',
                        style: const TextStyle(
                            color: Colors.white,
                            fontSize: 14,
                            fontWeight: FontWeight.bold)),
                    subtitle: Text(
                        'انضم في: ${_formatTimestamp(friend['joinedAt'])}',
                        style: const TextStyle(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 10),
          const Text(
              'أرسل رابطاً أنيقاً يحتوي على كودك الخاص وابدأ في جني الثمار الملكية فوراً',
              textAlign: TextAlign.center,
          ...
          Row(
            ...
            children: [
              _socialCircle(
                  Icons.message, Colors.green, () => _shareInvite(code, name)),
              const SizedBox(width: 20),
              _socialCircle(Icons.send_rounded, Colors.blue,
                  () => _shareInvite(code, name)),
              const SizedBox(width: 20),
              _socialCircle(Icons.facebook, Colors.blue.shade800,
                  () => _shareInvite(code, name)),
            ],
          ),
        ],
