...
class _AdminBadgesMgmtPageState extends State<AdminBadgesMgmtPage> {
  ...
  void _addBadge() async {
    ...
    if (_selectedImage == null && _iconController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('الرجاء اختيار صورة أو إدخال إيموجي')));
      return;
    }
      ...
      if (_selectedImage != null) {
        ...
        if (extension == '.gif') {
          final ref = FirebaseStorage.instance
              .ref()
              .child('badges/${DateTime.now().millisecondsSinceEpoch}.gif');
          final uploadTask = await ref.putFile(
              _selectedImage!, SettableMetadata(contentType: 'image/gif'));
          ...
          imageUrl = await MediaUploader.uploadCompressedImage(
            file: _selectedImage!,
            pathInStorage:
                'badges/${DateTime.now().millisecondsSinceEpoch}.jpg',
          );
        }
      }
          ...
          return Padding(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  const Text('إضافة شارة ملكية (تدعم GIF)',
                      style: TextStyle(
                          color: Colors.white,
                          fontSize: 18,
                  ...
                  TextField(
                      ...
                      decoration: const InputDecoration(
                          labelText: 'الأيقونة (إيموجي - اختياري)',
                          labelStyle: TextStyle(color: Colors.white70))),
                  TextField(
                  ...
                  DropdownButton<String>(
                    ...
                    items: const [
                      DropdownMenuItem(
                          value: 'badge', child: Text('فخري (متجر)')),
                      DropdownMenuItem(
                          value: 'achievement', child: Text('إنجازات (وطنية)')),
                      DropdownMenuItem(
                          value: 'activity', child: Text('نشاط (تفاعل)')),
                    ],
                    onChanged: (val) =>
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
            title: const Text('إدارة الشارات المتحركة'),
            backgroundColor: const Color(0xFF1A1A2E)),
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  subtitle: Text(
                      'السعر: ${badge['price']} | القسم: ${badge['category']}',
