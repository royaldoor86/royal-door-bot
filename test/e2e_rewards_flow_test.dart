...
void main() {
  ...
  group('E2E Rewards System Test', () {
    ...
    test('Full Scenario: Purchase -> Daily Harvest -> Transfer Gift', () async {
      debugPrint('--- Starting E2E Rewards Scenario Test ---');
      ...
      debugPrint('Step 1: Mock Users Created.');
      ...
      debugPrint(
          'Step 2: Package Purchased Successfully. ID: ${activeReward.id}');
      ...
      final currentGems =
          (updatedUserDoc.data()?['rewards_wallet_gems'] ?? 0.0) as double;

      debugPrint('Step 3: Daily Reward Harvested. New Balance: $currentGems');
      ...
      debugPrint('Step 4: Gift Transferred ($transferAmount gems).');
      ...
      final fundDoc = await FirebaseFirestore.instance
          .collection('global_support_fund')
          .doc('status')
          .get();

      final senderBalance =
          senderDocAfter.data()?['rewards_wallet_gems'] as double;
      final recipientBalance =
          recipientDocAfter.data()?['rewards_wallet_gems'] as double;
      final fundPool = fundDoc.data()?['current_gems_pool'] as double;

      debugPrint('Final Balances:');
      debugPrint('- Sender: $senderBalance');
      debugPrint('- Recipient: $recipientBalance');
      debugPrint('- Global Fund Pool: $fundPool');
      ...
      debugPrint('--- E2E Rewards Scenario Test Passed 100% ---');
    });
  });
}
